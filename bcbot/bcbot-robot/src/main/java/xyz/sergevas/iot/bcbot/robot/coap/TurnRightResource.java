...
public class TurnRightResource extends CoapResource {
	
private static final Logger LOG = Logger.getLogger(ForwardResource.class);
	
	private RobotMove robotMove;

	public TurnRightResource(String name, CamelContext camelContext) throws Exception {
		super(name);
		robotMove = new ProxyBuilder(camelContext).endpoint(ROBOT_MOVE_TURN_RIGHT_ROUTE).build(RobotMove.class);
	}
	...
	public void handlePUT(CoapExchange exchange) {
		...
		LOG.debug(format("Start Calling Camel route to turn the robot right with speed mode [%s]", robotSpeedMode));
