...
private static final Logger LOG = Logger.getLogger(ForwardResource.class);
	...
	public BackwardResource(String name, CamelContext camelContext) throws Exception {
		super(name);
		robotMove = new ProxyBuilder(camelContext).endpoint(ROBOT_MOVE_BACKWARD_ROUTE).build(RobotMove.class);
	}
	...
	public void handlePUT(CoapExchange exchange) {
		...
		LOG.debug(format("Start Calling Camel route to move the robot backward with speed mode [%s]", robotSpeedMode));
