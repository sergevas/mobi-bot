...
public class ForwardResource extends CoapResource {
	
	private static final Logger LOG = Logger.getLogger(ForwardResource.class);
	
	private RobotMove robotMove;

	public ForwardResource(String name, CamelContext camelContext) throws Exception {
		super(name);
		robotMove = new ProxyBuilder(camelContext).endpoint(ROBOT_MOVE_FORWARD_ROUTE).build(RobotMove.class);
	}
	...
	public void handlePUT(CoapExchange exchange) {
		...
		LOG.debug(format("Start Calling Camel route to move the robot forward with speed mode [%s]", robotSpeedMode));
